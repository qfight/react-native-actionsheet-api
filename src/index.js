...
export default class ActionSheet extends Component {
  ...
  render () {
    if (this.__should_not_render || this.state.sheets.length === 0) {
      return null;
    }
        ...
        {list.map((button, bIndex) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.9}>
              <Text style={[styles.buttonText, {color, fontWeight}]}>
                {title}
              </Text>
            </TouchableOpacity>
          );
...
ActionSheet.showActionSheetWithOptions = (
  ...
  if (isIOS) {
    ...
    if (global.__action_sheet instanceof ActionSheet) {
      ...
      throw Error('ActionSheet has not been initialized. To initialize ActionSheet, you should put `<ActionSheet />` to your render().');
    }
  }
};
